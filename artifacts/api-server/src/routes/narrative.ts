...
import { geminiGenerate, geminiStream, GEMINI_NARRATION_MODEL, GEMINI_STATE_MODEL } from "../gemini";

const router: IRouter = Router();

function isBudgetExceeded(err: any): boolean {
...
router.post("/generate", async (req, res) => {
  try {
    ...
    // PASO 1: Limitar historial reciente — NUNCA enviar historia completa
    ...
    // PASO 5 — LLAMADA 1: Generar narración únicamente (sin JSON)
    ...
    // PASO 5 — LLAMADA 2: Actualizar estado únicamente (sin prosa narrativa)
    ...
    const rawStateText = await geminiGenerate(
      GEMINI_STATE_MODEL,
      stateUserPrompt,
      stateSystemPrompt,
    );
    ...
    res.json(buildNarrativeResponse(narrative, meta));
  } catch (err) {
    console.error(err);
    if (isBudgetExceeded(err)) {
      return res.status(402).json({ error: "BUDGET_EXCEEDED", message: "El límite mensual gratuito de IA está agotado. Se renueva el 1 de mayo. Para continuar ahora, actualiza tu plan en Replit." });
    }
...
router.post("/stream", async (req, res) => {
  try {
    ...
    // PASO 5 — LLAMADA 1: Stream de narración únicamente
    const narrationSystemPrompt = buildNarrationSystemPrompt({
      gameConfig: gameConfig || {}, voice, tone, era: era || {}, character, currentTurn,
      isTimeJump, timeJumpYears,
    });
    ...
    // PASO 5 — LLAMADA 2: Actualización de estado (no streaming)
    const stateSystemPrompt = buildStateUpdateSystemPrompt();
    const stateUserPrompt = buildStateUpdateUserPrompt({
    ...
    const rawStreamState = await geminiGenerate(
      GEMINI_STATE_MODEL,
      stateUserPrompt,
      stateSystemPrompt,
    );
    ...
    // Escribir el bloque META en el mismo stream (compatible con el cliente)
    res.write('\n---META---\n');
    res.write(metaJson);
    res.end();
  } catch (err) {
...
Empieza directamente con el sueño, sin introducción. Usa lenguaje poético y evocador.`;
    ...
    const dreamText = await geminiGenerate(GEMINI_STATE_MODEL, prompt);
    res.json({ dream: dreamText || "Un sueño que se disuelve al despertar..." });
  } catch (err) {
    console.error(err);
...

{
  ...
  "fullStateSnapshot": {
    "character": "<personaje completo corregido>",
    "worldState": "<mundo completo corregido con ingameDate>",
    "realisticAttributes": "<atributos completos>",
    "descriptors": "<descriptores completos>",
    "inventory": "<inventario completo>",
    "currency": "<moneda completa>",
    "npcs": "<NPCs completos>",
    "facciones": "<facciones completas>",
    "customSections": "<secciones personalizadas completas>",
    "consequenceQueue": "<consecuencias completas>"
  },
  "summary": "Resumen de los cambios realizados en una oración."
}`;

    const stateRaw = await geminiGenerate(GEMINI_STATE_MODEL, prompt);
...
Solo JSON puro, sin markdown.`;
    ...
    const suggestRaw = await geminiGenerate(GEMINI_STATE_MODEL, prompt);
    let result: any = {};
    try {
    ...
    res.json({ ok: true, newValue: result.newValue, explanation: result.explanation });
  } catch (err) {
    console.error(err);
...
Escribe en tercera persona, tiempo pasado. Hazlo sentir como un epitafio o registro histórico. Sé específico sobre quién fue esta persona.`;
    ...
    const summaryText = await geminiGenerate(GEMINI_STATE_MODEL, prompt);
...
{ "resumen": "<nuevo resumen completo aquí>" }`;
    ...
    const memoryRaw = await geminiGenerate(GEMINI_STATE_MODEL, prompt);
    let resumen = '';
    try {
