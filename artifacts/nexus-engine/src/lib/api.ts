...
export async function generateNarrativeStream(
  ...
  if (!res.ok) {
    ...
    if (res.status === 402 || errBody?.error === 'BUDGET_EXCEEDED') {
      throw new Error('BUDGET_EXCEEDED:' + (errBody?.message || ''));
    }
    throw new Error('Narrative stream failed');
  }
...
export async function suggestFieldValue(payload: { fieldPath: string; currentValue: any; suggestion: string; context: any }): Promise<{ newValue: any; explanation: string }> {
