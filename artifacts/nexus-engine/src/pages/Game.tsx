...
export default function Game() {
  ...
  const handleFirstNarration = async () => {
    ...
    try {
      ...
      if (errMsg.startsWith('BUDGET_EXCEEDED:')) {
        ...
        updateLastNarrativeTurn({ text: `⚠ Límite mensual de IA agotado.\n\n${detail || 'El límite gratuito se renueva el 1 de mayo. Para continuar ahora, actualiza tu plan en replit.com.'}` });
      } else {
  ...
  const applyNarrativeResult = useCallback((result: Awaited<ReturnType<typeof generateNarrativeStream>>, currentRun: typeof run) => {
    ...
    const snapshot: any = (result as any).fullStateSnapshot || (result as any).stateSnapshot || (result as any).fullState || null;
    ...
    const it = result.ingameTime || snapshot?.ingameTime || {};
    const calculatedYear = startYear + yearsElapsed;
    const newIngameYear = it.year ?? snapshot?.worldState?.ingameYear ?? calculatedYear;
    const birthYear = snapshot?.character?.birthYear ?? currentRun.character?.birthYear ?? startYear;
    const newAge = snapshot?.character?.age ?? Math.max(0, newIngameYear - birthYear);
    const wsu = mergeDefined(snapshot?.worldState ? { ...snapshot.worldState } : {}, result.worldStateUpdates || {});
    ...
    const ingameDateStr = it.ingameDate || wsu.ingameDate || (newDay && newMonth ? String(newDay) + ' de ' + MESES_MEDIEVALES[(newMonth - 1) % 12] + ', ' + String(newIngameYear) : 'Año ' + String(newIngameYear));

    let nextRun: any = {
      ...currentRun,
      ...(snapshot ? withoutNulls(snapshot) : {}),
      ...
      inventory: snapshot?.inventory || [...(currentRun.inventory || [])],
      currency: snapshot?.currency || currentRun.currency,
      npcs: snapshot?.npcs || [...(currentRun.npcs || [])],
      facciones: snapshot?.facciones || [...(currentRun.facciones || [])],
      personalHistory: snapshot?.personalHistory || [...(currentRun.personalHistory || [])],
      consequenceQueue: snapshot?.consequenceQueue || [...(currentRun.consequenceQueue || [])],
      traumas: snapshot?.traumas || [...(currentRun.traumas || [])],
      realisticAttributes: { ...(currentRun.realisticAttributes || {}), ...(snapshot?.realisticAttributes || {}) },
      descriptors: { ...(currentRun.descriptors || {}), ...(snapshot?.descriptors || {}) },
      customSections: snapshot?.customSections || [...(currentRun.customSections || [])],
    };
    ...
    try {
      ...
      if (shouldUpdateMemory) {
        ...
        updateAIMemory({
          ...
          updateMemoriaNarrador({ resumen } as any);
        }).catch(() => {});
      }
...
function EditorPanel({ run, onRegenerate, isGenerating, canRegenerate }: {
  ...
  const [tab, setTab] = useState(0);
  const TABS = ['Narración', 'Contenido', 'Experiencia', 'Ritmo', 'Simulación'];
